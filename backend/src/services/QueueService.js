...
// Exponential backoff with jitter: delay = base * 2^attempt + random(0..1000) ms
const buildBackoff = (base = 1000, maxDelay = 30_000) => ({
  type: 'custom',
  settings: {},
  // BullMQ custom backoff: receives (attemptsMade) → delay ms
  strategy: (attemptsMade) => {
    const exp = Math.min(base * Math.pow(2, attemptsMade), maxDelay);
...
class QueueService {
  constructor() {
    ...
    /** Whether Redis is available and queues are active */
    this.enabled = false;
  }
    ...
    if (!this._queues.has(name)) {
      const queue = new Queue(name, {
        ...
        defaultJobOptions: {
          ...
          removeOnComplete: { count: 100, age: 86400 },   // keep last 100 or 1 day
          removeOnFail: { count: 200, age: 7 * 86400 },   // keep last 200 or 7 days
        },
        ...options,
      });
   ...
   * No-op when disabled; returns null.
   *
  ...
  /**
   * Register a repeatable (scheduled) job.
  ...
  async getHealth() {
    if (!this.enabled) {
      return { status: 'degraded', redis: false, queues: [] };
    }
